...
def save_plots(data_dic, epoch, args, acc=0.5):
    ...
    if args.test_only:
        plot_learning_curve([data_dic["test_accuracy"]],
                            ...
                            labels=['validation'],
                            save_name=args.results_dir + '/{}-learning_curve_epoch_{}_acc_{:.4f}'.format(args.data_source, epoch, acc))

        plot_learning_curve([data_dic["test_loss"]], hlines=[],
                            xlabel='training epochs/{}'.format(args.test_freq),
                            ylabel='loss', title='Loss', colors=['limegreen'],
                            labels=['validation'],
                            save_name=args.results_dir + '/{}-loss_epoch_{}_acc_{:.4f}'.format(args.data_source, epoch, acc))
...
# -----------------need ------------------
def plot_confusion_matrix(args, confm, title='Confusion matrix', cmap='Blues', normalize=False, postfix=0.5):  # plt.cm.Blues):
    """
    ...
    plt.savefig(os.path.join(args.results_dir,'certain_uncertain', '{}-val-confusion-acc-{}.png'.format(args.data_source, postfix)), format='png')
            ...
            if len(need_inds) > 1:
                ...
                for jj in range(len(need_inds)//(plots_per_fig)+1):
                    ...
                    for count, vis_h, ori, label, pred_lb \
                        ...
                        att_indices = collect_and_plot_atten(vis_h, args.sr, np.max(ori))  # collect the attention part indices
                        ...
                        if count == 0:
                            ...
                            plt.legend(bbox_to_anchor=(0.1, 1.05, 2, 0.1), frameon=False, ncol=min(5, args.num_classes+1))
                        plt.setp(ax.get_yticklabels(), visible=False)
                    ...
                    save_name = args.results_dir + '/attention_maps/cam-trueclass_{}-fig{}-{}-{}'.format(class_id, jj, postfix, args.data_source)
                    plt.savefig(save_name+".png", format='png')
                    plt.savefig(save_name+".pdf", format='pdf')
                    plt.close()#
            

def collect_and_plot_atten(indices, scale, ymax):
    """
    ...
    if len(indices) < 1:
        ...
        # for ind, xc in enumerate(start_end_inds):
        #     if ind % 2 == 0:
        #         plt.hlines(ymax, (indices[start_end_inds[ind]] / np.float(scale)), (indices[start_end_inds[ind+1]] / np.float(scale)), color='c', linewidth=1)
        ...
        if len(plot_inds) > 0:
            ...
            if len(labels_int[labels_int == class_id]) != 0:
                for ind in range(num_figs):
                    ...
                    plt.suptitle("{}-Very certain examples of class {}".format(args.data_source, class_names[class_id]), x=0.5, y=0.925, fontsize=base)
                    if not args.if_spectrum:
                        ...
                        for j in range(ind*row*col, min((ind+1)*row*col, features_plot.shape[0]-1)):
                            ...
                            axs[(j-ind*row*col) // col, np.mod(j, col)].plot(np.arange(features_plot.shape[1]) / np.float(args.sr), features_plot[j, :], color=color)
                        ...
                        f.savefig(
                            args.results_dir +
                            '/certain_uncertain/{}-certain_examples_of_class{}_acc_{:.3f}_fig_{}-{}-{}.png'.format(args.data_source, class_id, acc, ind, args.class_mode, postfix))
                        f.savefig(
                            args.results_dir +
                            '/certain_uncertain/{}-certain_examples_of_class{}_acc_{:.3f}_fig_{}-{}-{}.pdf'.format(args.data_source, class_id, acc, ind, args.class_mode, postfix), format="pdf")
                        plt.close()
                    else:
                        for j in range(ind*row*col, min((ind+1)*row*col, features_plot.shape[0]-1)):
                            axs[(j-ind*row*col) // col, np.mod(j, col)].imshow(features_plot[j,...].T, interpolation="nearest", cmap='viridis', aspect="auto", origin='lower')
                        ...
                        f.text(0.1, 0.5, 'Normalized amplitude',
                               rotation=90, fontsize=base,
                               verticalalignment='center')
                        plt.savefig(args.results_dir + '/certain_uncertain/Most_certain_samples_acc_{:.3f}_fig_{}-{}.png'.format(acc, ind, args.class_mode), format='png')
                        plt.savefig(args.results_dir + '/certain_uncertain/Most_certain_samples_acc_{:.3f}_fig_{}-{}.pdf'.format(acc, ind, args.class_mode), format='pdf')
                        plt.close()
...
def plot_bin_roc_curve(args, data, acc=0.5, postfix="seg-level-AUC"):
    ...
    fpr, tpr, _ = metrics.roc_curve(np.argmax(data["test_labels"], 1), data["test_pred_logits"][:, 1])  # input the positive label's prob distribution
    auc = metrics.auc(fpr, tpr)
    plt.plot(fpr, tpr, label="auc={0:.4f}".format(auc))
    plt.legend(loc=4)
    plt.xlabel("false positive rate")
    plt.ylabel("true positive rate")
    plt.savefig(args.results_dir + '/certain_uncertain/{}-AUC-curve-acc-{:.3f}-auc-{:.4f}-{}.png'.format(args.data_source, acc, auc, postfix), format="png")
    plt.savefig(args.results_dir + '/certain_uncertain/{}-AUC-curve-acc-{:.3f}-auc-{:.4f}-{}.eps'.format(args.data_source, acc, auc, postfix), format="eps")
    plt.close()
...
def plot_multiclass_roc(args, data, acc=0.5, postfix="seg-level-AUC"):
    ...
    plt.savefig(args.results_dir + '/certain_uncertain/{}-{}-acc-{:.3f}-auc-{:.4}.png'.format(args.data_source, postfix, acc, roc_auc["micro"]))
    plt.close()
...
def plot_sample_w_recon(data, recon=None, sr=512, num_figs=5, smp_names=None, ifpdf=False, step=0, save_dir="results/", postfix="train"):
    """
    :param data:
    :param diff:
    :param recon:
    ...
    for ind in range(num_figs):
        f, axs = plt.subplots(row, col, sharex=True)
        plt.suptitle("\n".join(wrap("Samples and Reconstructions\n{}".format(postfix), 60)), x=0.5, y=0.98, fontsize=base)
        
        for j, ori, recon in zip(np.arange(row * col), data_plot[ind*row*col: (ind+1)*row * col], recon_plot[ind*row*col: (ind+1)*row * col]):
            ...
            axs[j// col, np.mod(j, col)].plot(np.arange(recon.size)/sr, recon, "c", label="reconstruction", linewidth=0.8)
            axs[0, 0].legend(bbox_to_anchor=(0.15, 1.05, 2, 0.1), loc="lower left", mode="expand", frameon=False, borderaxespad=0, ncol=2)
            if np.mod(j, col) > 0:
        ...
        if smp_names is not None:
            ...
            np.savetxt(os.path.join(save_dir, '{}_recon_diff_ep{}_fns_{}x{}.csv'.format(postfix, step, row, col)), np.array(fnames).reshape(-1,1), fmt="%s", delimiter=",")
        if ifpdf:
