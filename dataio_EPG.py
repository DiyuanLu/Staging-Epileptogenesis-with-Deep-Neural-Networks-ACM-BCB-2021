...
def find_files(args):
		...
		for root, dirnames, fnames in os.walk(folder):
			...
			if args.class_mode == "BL+1EPG":
				...
				if os.path.basename(root) == "EPG" or os.path.basename(root) == "STIM":
					...
					if len(fnames) > 1:
						split_stages = get_stage_timestamps(fnames, num_stages=num_EPGs, days4train=args.days4train,
						                                    last_day=args.end_EPG_date[rat_id])
	...
	if args.test_only:
		time_stamps = np.array([get_timestamp_from_file(os.path.basename(fn), year_ind=args.year_ind) for fn in
		                        np.array(test_files_labels)[:, 0]])
	...
	if args.test_ratio != 1:  # when it is not in test_only case, there are training files
		np.savetxt(os.path.join(args.results_dir, "train_files-{}.txt".format(args.data_source)),
		           np.sort(np.array(train_files_labels)), fmt="%s", delimiter=",")
...
def get_train_test_files_split(root, fns, ratio, train_list, test_list, year_ind=1, rat_id="1227", label=0,
                               num2use=100):
	"""
	...
	time_stamps = np.array(
			[get_timestamp_from_file(os.path.basename(fn), year_ind=year_ind) for fn in
			 np.array(current_test_files)[:, 0]])
...
def get_spectrum(features, label, filename, id, args=None):
	...
	try:  # for some segments, the remaining secs are not enough for a segment
		...
		power_spec = tf.compat.v1.real(
			stfts * tf.compat.v1.conj(stfts))  # A float32 Tensor of shape [batch_size, time_bins, fft_bins].
...
def check_make_dirs(args):  # picked-300h-11seg-1237
	...
	if train_or_test == "test":
		...
		args.results_dir = os.path.join(os.path.dirname(pretrained_dir), "-".join(
			os.path.basename(pretrained_dir).split("-"))) + '-{}-{}h-{}-test'.format(short_time_str,
		                                                                             args.num_hours_per_class,
		                                                                             args.data_source)
	else:
		args.results_dir = os.path.join(args.results_root,
		                                time_str + args.class_mode + '-' + args.model_name + '-' + args.postfix + "-{}".format(
			                                train_or_test))
...
def load_model(saver, sess, model_dir):
	...
	if ckpt:
		...
		saver.restore(sess,
		              os.path.join(os.path.dirname(model_dir), "model", os.path.basename(ckpt.model_checkpoint_path)))
		print(' Done.')
		return global_step
	else:
